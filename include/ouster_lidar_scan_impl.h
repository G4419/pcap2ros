...
#include "ParsesPcap/ouster_lidar_scan.h"
#include "ParsesPcap/ouster_types.h"
...
struct FieldTag<uint8_t> {
    static constexpr ChanFieldType tag = ChanFieldType::UINT8_OUSTER;
};

template <>
struct FieldTag<uint16_t> {
    static constexpr ChanFieldType tag = ChanFieldType::UINT16_OUSTER;
};

template <>
struct FieldTag<uint32_t> {
    static constexpr ChanFieldType tag = ChanFieldType::UINT32_OUSTER;
};

template <>
struct FieldTag<uint64_t> {
    static constexpr ChanFieldType tag = ChanFieldType::UINT64_OUSTER;
};
...
struct FieldSlot {
    ...
    FieldSlot(ChanFieldType t, size_t w, size_t h) : tag{t} {
        switch (t) {
            case ChanFieldType::VOID_OUSTER:
                break;
            case ChanFieldType::UINT8_OUSTER:
                new (&f8) img_t<uint8_t>{h, w};
                f8.setZero();
                break;
            case ChanFieldType::UINT16_OUSTER:
                new (&f16) img_t<uint16_t>{h, w};
                f16.setZero();
                break;
            case ChanFieldType::UINT32_OUSTER:
                new (&f32) img_t<uint32_t>{h, w};
                f32.setZero();
                break;
            case ChanFieldType::UINT64_OUSTER:
    ...
    FieldSlot() : FieldSlot{ChanFieldType::VOID_OUSTER, 0, 0} {};
...
void visit_field(SCAN&& ls, sensor::ChanField f, OP&& op, Args&&... args) {
    switch (ls.field_type(f)) {
        case sensor::ChanFieldType::UINT8_OUSTER:
            op.template operator()(ls.template field<uint8_t>(f),
                                   std::forward<Args>(args)...);
            break;
        case sensor::ChanFieldType::UINT16_OUSTER:
            op.template operator()(ls.template field<uint16_t>(f),
                                   std::forward<Args>(args)...);
            break;
        case sensor::ChanFieldType::UINT32_OUSTER:
            op.template operator()(ls.template field<uint32_t>(f),
                                   std::forward<Args>(args)...);
            break;
        case sensor::ChanFieldType::UINT64_OUSTER:
