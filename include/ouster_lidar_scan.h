...
class LidarScan {
   public:
    ...
    using raw_t = uint32_t;                 ///< @deprecated
    using ts_t = std::chrono::nanoseconds;  ///< @deprecated
    ...
    using Field = sensor::ChanField;  ///< @deprecated
 ...
 * @return xyz direction and offset vectors for each point in the lidar scan.
 */
__declspec(dllexport) inline XYZLut make_xyz_lut(const sensor::sensor_info& sensor) {
    return make_xyz_lut(
...
#include "ParsesPcap/ouster_lidar_scan_impl.h"
