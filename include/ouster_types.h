 ...
 */
enum NMEABaudRate {
    BAUD_OUSTER_9600 = 1,  // 9600 bits per second UART baud rate
    BAUD_OUSTER_115200     // 115200 bits per second UART baud rate
};
 ...
 */
struct sensor_info {
    std::string name;               ///< @deprecated Will be removed in the next version
 ...
 */
enum ChanFieldType {VOID_OUSTER = 0, UINT8_OUSTER, UINT16_OUSTER, UINT32_OUSTER, UINT64_OUSTER};
 ...
 */
class __declspec(dllexport) packet_format final {
    template <typename T>
